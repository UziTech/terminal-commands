		...
		return (e) => {
			...
			const fullCommands = commandsArray.map(cmd => {
				...
				if (this.filesPlaceholder) {
					fullCmd = fullCmd.replace(this.filesPlaceholder, files.join(" "));
				}
				if (this.filePlaceholder) {
					fullCmd = fullCmd.replace(this.filePlaceholder, files[0]);
				}
				if (this.dirPlaceholder) {
					fullCmd = fullCmd.replace(this.dirPlaceholder, dir);
				}
				return fullCmd;
			});
			if (this.terminal) {
