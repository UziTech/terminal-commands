...
export default {
	...
	loadConfig() {
		...
		for (const command in this.json) {
			const commands = this.json[command];
			this.commandSubscriptions.add(atom.commands.add("atom-workspace", command, this.runCommands(commands)));
	...
	runCommands(commands) {
		...
		return (e) => {
			...
			const dir = (files[0] ? path.dirname(files[0]) : null);
			const [project] = (files[0] ? atom.project.relativizePath(files[0]) : null);
			...
			const fullCommands = commandsArray.map(cmd => {
				...
				if (this.filesPlaceholder) {
					if (!files && replacedCmd.match(this.filesPlaceholder)) {
				...
				if (this.filePlaceholder) {
					if (!files[0] && replacedCmd.match(this.filePlaceholder)) {
