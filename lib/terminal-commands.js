...
import { CompositeDisposable, Disposable, watchPath, Emitter } from "atom";
...
export default {
	...
	async createConfig() {
		try {
			const defaultConfig = await util.promisify(fs.readFile)(path.resolve(__dirname, "./default-config.json"));
			await util.promisify(fs.writeFile)(this.configFile, defaultConfig);
		} catch (err) {
	...
	async configExists() {
		try {
			await util.promisify(fs.access)(this.configFile);
			return true;
		} catch (err) {
			return false;
		}
	...
	requireConfigFile() {
		...
		try {
			...
			this.removeCache(this.configFile);
		} catch (err) {
	...
	runCommands(commands) {
		...
		return (e) => {
			const files = this.getPaths(e && e.target);
			...
			if (Object.keys(errors).length > 0) {
				...
				this.terminals[0].run(fullCommands);
			} else {
				atom.notifications.addError("No terminal loaded", {
					description: "Please install a terminal package.  \nWe recommend [`x-terminal`](https://atom.io/packages/x-terminal)."
				});
			}
		};
	},
